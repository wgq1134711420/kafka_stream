...
if __name__ == '__main__':
    mysql = PymysqlPool("desktop")
    # sqlAll = "select * from seeyii.timer_task_list;"
    # result = mysql.getAll(sqlAll)
    # print(result)
    #
    sqlAll = "select * from EI_BDP.fsp_branch_tree;"
    result = mysql.getMany(sqlAll, 2)
    print(result)
    # sql = "select * from ei_bdp.company_increase_logs where colname='chuandong_company' order by gtime DESC;"
    # result = mysql.getOne(sql)
    # print(result)

    # mysql.insert("insert into seeyii.mytest set filename=%s", (1))
